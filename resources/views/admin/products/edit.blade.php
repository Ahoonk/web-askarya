...
@section('content')
    @php
        $featuresText = old('features', implode(PHP_EOL, $product->features ?? []));
    @endphp
    ...
    <div class="space-y-6">
        <div class="rounded-[2rem] border border-white/10 bg-white/5 p-8">
            ...
            <h1 class="mt-3 font-display text-4xl font-bold text-white">Edit product</h1>
        </div>

        <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-6">
            @csrf
            @method('PUT')
            <div class="grid gap-6 lg:grid-cols-2">
                <div class="space-y-4">
                    <div>
                        ...
                        <input name="name" value="{{ old('name', $product->name) }}" class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white" />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm text-slate-300">Sort order</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', $product->sort_order) }}" class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white" />
                    </div>
                    <div class="lg:col-span-2">
                        <label class="mb-2 block text-sm text-slate-300">Description</label>
                        <textarea name="description" rows="4" class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white">{{ old('description', $product->description) }}</textarea>
                    </div>
                    <div class="lg:col-span-2">
                ...
                <div class="rounded-[1.5rem] border border-white/10 bg-slate-950/60 p-5">
                    ...
                    <p class="mt-2 text-sm text-slate-400">Ganti atau hapus gambar produk dari sini.</p>

                    @if ($product->image_url)
                        <div class="mt-4 overflow-hidden rounded-2xl border border-white/10">
                            <img src="{{ $product->image_url }}" class="h-48 w-full object-cover" alt="{{ $product->name }}">
                        </div>
            ...
            <div class="mt-5 flex gap-3">
                <button type="submit" class="rounded-full bg-cyan-400 px-6 py-3 font-bold text-slate-950 transition hover:bg-cyan-300">Update Product</button>
                <a href="{{ route('admin.products.index') }}" class="rounded-full border border-white/10 px-6 py-3 font-semibold text-white transition hover:bg-white/5">Back</a>
            </div>
        </form>
    </div>
@endsection
